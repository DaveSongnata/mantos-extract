...
# "Mantos Extract"), consumed by MantosExtract.Resources.dll so the CorelDRAW toolbar button
# shows the mark. Same byte-level recipe as ../../../optimus/src/Optimus.Resources/build_res.ps1
# (not shared — each addin owns its copy).
...
$frames = @(
  @{ id=1; png=(Join-Path $iconsDir 'mantosextract-16.png') },
  @{ id=2; png=(Join-Path $iconsDir 'mantosextract-24.png') },
  @{ id=3; png=(Join-Path $iconsDir 'mantosextract-32.png') },
  @{ id=4; png=(Join-Path $iconsDir 'mantosextract-48.png') }
)
$dibs = @()
foreach ($f in $frames) {
